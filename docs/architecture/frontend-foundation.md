...
지도 URL은 선택한 권역 또는 단지의 안정적인 공개 ID, 지도 중심 좌표, zoom과 활성
필터만 담는다. cluster ID, 검색어 입력 중 값, hover·로딩·자동완성 같은 일시적 UI
상태는 URL에 저장하지 않는다. 사용자가 지도를 연속으로 이동하거나 확대·축소하면
이동 종료 시점에 현재 history 항목을 교체한다. 검색 결과·권역·단지를 명시적으로
선택하면 새 history 항목을 추가해 브라우저 뒤로가기로 이전 선택 상태를 복원한다.
